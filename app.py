...
def simulate_what_if(machine_code, temperature, vibration, load, voltage, current,
    ...
    status = "running"
    if sim_temp > 90 or sim_vib > 12:
        status = "stopped"
    elif sim_temp > 78 or sim_vib > 7:
        status = "maintenance"

    reading = standard_reading(machine_code, sim_temp, sim_vib, sim_load, 0, voltage, current, status)
    ...
    return {
        ...
        "deltas": {"temperature_pct": temp_delta_pct, "vibration_pct": vib_delta_pct, "load_pct": load_delta_pct},
        "status": status,
...
def send_email(to_email, subject, body):
    """Best-effort SMTP send to an arbitrary recipient. Returns True/False so callers
    (like the password-reset flow) can tell the user if email delivery isn't configured."""
    if not SMTP_HOST:
        print(f"SMTP not configured - email to {to_email} was not sent.")
        return False
    try:
        import smtplib
        ...
        print(f"Email to {to_email} failed to send:", e)
        return False
...
def api_forgot_password():
    ...
    sent = send_email(user.email, tpl["subject"], tpl["body"].format(code=code))

    # Dev/demo fallback: without SMTP configured, surface the code directly so
  ...
  document.getElementById("register-form").addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("/api/register", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ full_name, email, password, confirm_password, role: document.getElementById("reg-role").value }),
      });
      const data = await res.json();
      if (!res.ok) {
